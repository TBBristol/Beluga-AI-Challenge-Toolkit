...
#Rendering
        ...
        self.true_observation_space = BoxSpace(low = -1, high = 1000, shape = (20,13)) #20 racks 13 features
        ...
        self.action_space = BoxSpace(low = -1, high = 1, shape = (20,)) #Select a rack to put on (probability) TODO check if this is correct
    ...
    @property
    def delivery_priorities(self):
        return self.get_delivery_priorities()
    @property
    ...
    def get_unload_list(self, initial_unloads):
            ...
            while not next_unload == 'dummy-jig':
                for next_unload_pair in next_unload_set:
                    if self.obj_mapping[next_unload_pair[0]] == next_unload:
                        ...
                        next_unload_set.remove(next_unload_pair)
                        break
        #current_unloads is the next one to come of each flight
        ...
        for k,v in current_unloads.items():
            #Take the total list and snip it before the current unload
            unload_list[k] = unload_list[k][unload_list[k].index(v):]
        return unload_list
    

    def get_delivery_priorities(self):
        priorities = {}
        for k,v in self.delivery_list.items():
            priority = 1
            for i in v:
                if i != "dummy-jig":
    ...
    def flattened_unload_list(self):
        return [i for k in self.flight_list  for i in self.unload_list[k] if i != "dummy-jig"]
    ...
    def get_flight_list(self, initial_flight):
        ...
        #Snip prior to current flight
        curr_flight = self.current_pddl_state.atoms[self.pred_mapping['processed-flight']][0][0]
        return [self.obj_mapping[f] for f in flight_list][flight_list.index(curr_flight):]
